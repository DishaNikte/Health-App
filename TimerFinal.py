...
def clock():
    ...
    curr_time.after(1000,clock)  #the timeout; the duraton after which it shows the actul time 
curr_time =Label(root, font ='arial 15 bold', text = '', fg = 'gray25' ,bg ='blanched almond')
curr_time.place(x = 300 , y = 170)
clock()
sec = StringVar()     #textvariable-when it is going to change multiple times in a program
...
def countdown():
    times = int(hrs.get())*3600+ int(mins.get())*60 + int(sec.get())   #get method - get values from entry box
    while times > -1:
     minute,second = (times // 60 , times % 60)    #integer value is returned after division
